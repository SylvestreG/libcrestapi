...
hosts const &api::get_hosts(void) const { return _hosts; }

services const &api::get_services(void) const { return _services; }

hostgroups const &api::get_hostgroups(void) const { return _hostgroups; }
servicegroups const &api::get_servicegroups(void) const {
...
bool api::fetch(std::string const &url, std::string const &login,
  ...
  std::string token;
  {
    json11::Json jsq{json11::Json::object{
        {"security",
         json11::Json::object{
             {"credentials",
              json11::Json::object{{"login", login}, {"password", pass}}}}}}};
...
void api::fill_hosts(RestClient::Connection &conn, uint32_t page) {
  std::stringstream ss;
  uint32_t limit{100};
  ss << "/monitoring/hosts?limit=" << limit << "&"
     << "page=" << page;
...
void api::fill_services(RestClient::Connection &conn, uint32_t page) {
  std::stringstream ss;
  uint32_t limit{100};
  ss << "/monitoring/services?limit=" << limit << "&"
     << "page=" << page;
  RestClient::Response r = conn.get(ss.str());
